...
//    resize(src.width, src.height, GL2.GL_RGBA8, GL2.GL_RGBA, GLES3.GL_UNSIGNED_BYTE, 4, 1);
    resize(src.width, src.height, GLES30.GL_RGBA16F, GLES30.GL_RGBA, GLES30.GL_FLOAT, 4, 2);
  }
...
//      hi = (int) Math.ceil(hi * 0.5f);
      tex_temp[i].resize(context, internal_format, wi, hi, format, type, GLES30.GL_LINEAR, GLES30.GL_CLAMP_TO_EDGE, num_channels, bbp);
      tex_blur[i].resize(context, internal_format, wi, hi, format, type, GLES30.GL_LINEAR, GLES30.GL_CLAMP_TO_EDGE, num_channels, bbp);
      wi = (int) Math.ceil(wi * 0.5f);
      hi = (int) Math.ceil(hi * 0.5f);
    }
  }
