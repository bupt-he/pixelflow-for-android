...
public class MinMaxGlobal {
  ...
  public void resize(PGraphicsOpenGL pg){
    resize(GLES30.GL_RGBA8, pg.width, pg.height, GLES30.GL_RGBA, GLES30.GL_UNSIGNED_BYTE, 4, 1);
  }
  ...
  public void resize(int iformat, int w, int h, int format, int type, int num_channel, int byte_per_channel){
    ...
    for(int i = 0; i < layers; i++){
      ...
      tex[i].resize(context, iformat, w, h, format, type, GLES30.GL_NEAREST, GLES30.GL_MIRRORED_REPEAT, num_channel, byte_per_channel);
