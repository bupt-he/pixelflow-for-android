...
public class DwFlowField {
  ...
  int tex_wrap = GLES30.GL_CLAMP_TO_EDGE;
  ...
  public boolean resize(int w, int h){
    int internalFormat   = GLES30.GL_RG16F;
    int byte_per_channel = 2;
    if(param.HIGHP_FLOAT){
      internalFormat   = GLES30.GL_RG32F;
      byte_per_channel = 4;
    }
    boolean resized = tex_vel.resize(context, internalFormat, w, h, GLES30.GL_RG, GLES30.GL_FLOAT, GLES30.GL_LINEAR, tex_wrap, 2, byte_per_channel);
    if(resized){
      tex_vel.clear(0);
    }
    return resized;
  ...
  public void blur(int iterations, int radius){
    ...
    tex_tmp.setParamWrap(GLES30.GL_MIRRORED_REPEAT);
    tex_vel.setParamWrap(GLES30.GL_MIRRORED_REPEAT);
  ...
  private void resizeLic(int w, int h){
    tex_lic.resize(context, GLES30.GL_RGBA8, w, h, GLES30.GL_RGBA, GLES30.GL_UNSIGNED_BYTE, GLES30.GL_LINEAR, GLES30.GL_MIRRORED_REPEAT, 4, 1);
