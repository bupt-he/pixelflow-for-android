...
public class DwGLTexture3D{
  ...
  public final int target     = GLES30.GL_TEXTURE_3D;
  ...
  public void release(){
    ...
    // release texture
    if(isTexture()){
      GLES32.glDeleteTextures(1, HANDLE, 0);
      HANDLE[0] = 0;
    ...
    // release pbo
    if(hasPBO()){
      GLES32.glDeleteBuffers(1, HANDLE_pbo, 0);
      HANDLE_pbo[0] = 0;
    }
...
//    System.out.println("GL_PACK_ALIGNMENT "+val[0]);
      ...
      GLES32.glTexImage3D   (target, 0, internalFormat, w, h, d, 0, format, type, data);
//    gl.glTexSubImage2D(target, 0, 0, 0, w, h, format, type, data);
    ...
    {
      GLES30.glBindTexture(target, HANDLE[0]);
      GLES30.glTexParameteri(target, GLES30.GL_TEXTURE_WRAP_R, wrap);
  ...
  public void setParam_Border(float[] border){
    GLES30.glBindTexture   (target, HANDLE[0]);
    GLES30.glTexParameterfv(target, GLES32.GL_TEXTURE_BORDER_COLOR, border, 0);
    GLES30.glBindTexture   (target, 0);
  }
  ...
  public void swizzle(int[] i4_GL_TEXTURE_SWIZZLE_RGBA){
    glTexParameteriv(GLES30.GL_TEXTURE_SWIZZLE_R, i4_GL_TEXTURE_SWIZZLE_RGBA, true);
  }
  ...
  private DwGLTexture3D createTexSubImage(int x, int y, int w, int h){
    ...
    GLES32.glCopyTexSubImage3D(target, 0, 0, 0, 0, x, y, w, h);
    GLES30.glBindTexture(target, 0);
...
//    ByteBuffer bbuffer = gl.glMapBuffer(GL2ES3.GL_PIXEL_PACK_BUFFER, GL2ES3.GL_READ_ONLY);
  ...
  public void getData_GL2GL3(Buffer buffer){
    ...
    GLES30.glBindTexture(target, HANDLE[0]);
    // TODO: 2023/7/1
//    GLES32.glTexImage3D(target, 0, format, type, buffer);
    GLES30.glBindTexture(target, 0);
