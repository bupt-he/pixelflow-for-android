...
import processing.filter.DistanceTransform;
import processing.filter.Merge;
...
public class DwFlowFieldParticles{
  ...
  public void createSpriteTexture(int size, float e1, float e2, float mult){
    context.begin();
    param.tex_sprite.resize(context, GLES30.GL_RGBA8, size, size, GLES30.GL_RGBA, GLES30.GL_UNSIGNED_BYTE, GLES30.GL_LINEAR, 1, 4);
...
//    int w_sum = w,                            h_sum = h;
    ...
    resized |= tex_obs_FG.resize(context, GLES30.GL_RGBA, w_obs, h_obs, GLES30.GL_RGBA, GLES30.GL_UNSIGNED_BYTE, GLES30.GL_NEAREST, 4, 1);
    resized |= tex_obs   .resize(context, GLES30.GL_RGBA, w_obs, h_obs, GLES30.GL_RGBA, GLES30.GL_UNSIGNED_BYTE, GLES30.GL_NEAREST, 4, 1);
    
    boolean HIGHP_FLOAT = true;
    int format  = GLES30.GL_RED;
    int filter  = GLES30.GL_LINEAR;
    int wrap    = GLES30.GL_CLAMP_TO_EDGE;
    int iformat = HIGHP_FLOAT ? GLES30.GL_R32F : GLES30.GL_R16F;
    int type    = HIGHP_FLOAT ? GLES30.GL_FLOAT : GLES30.GL_HALF_FLOAT;
  ...
  public void resizeParticlesCount(int w, int h){
    boolean resized = tex_particle.resize(context, GLES30.GL_RGBA32F, w, h, GLES30.GL_RGBA, GLES30.GL_FLOAT, GLES30.GL_NEAREST, 4, 4);
    if(resized){
      reset();
    }
  }
  ...
  public void createCollisionFlowField(){
    ...
    context.begin();
    {
      ...
      GLES30.glBlendFunc(GLES30.GL_SRC_COLOR, GLES30.GL_ONE); // ADD
  ...
  public final Merge.TexMad tm_acc = new Merge.TexMad();
  public final Merge.TexMad tm_col = new Merge.TexMad();
  public final Merge.TexMad tm_coh = new Merge.TexMad();
  public final Merge.TexMad tm_obs = new Merge.TexMad();
